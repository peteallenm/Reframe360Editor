...
ColorGrade::ColorGrade(QObject *parent)
    ...
    , m_saturation(1.0)
    , m_pop(0.0)
    , m_brightLows(0.0), m_brightMids(0.0), m_brightHighs(0.0)
...
void ColorGrade::setBrightMids(double v) { if (qFuzzyCompare(m_brightMids, v)) return; m_brightMids = v; emit brightMidsChanged(); }
